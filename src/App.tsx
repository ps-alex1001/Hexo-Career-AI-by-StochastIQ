...
import { analyzeResumeAndJob, AnalysisResponse } from './services/geminiService';
import { cn } from './lib/utils';
import { motion } from 'motion/react';

type AppState = 'landing' | 'workspace' | 'dashboard' | 'blueprint' | 'how-it-works';
...
export default function App() {
  ...
  const [activeFooterModal, setActiveFooterModal] = useState<'privacy' | 'terms' | 'changelog' | 'status' | null>(null);
  ...
  const startAnalysis = async (resume: string, targetType: string, targetContent: string, githubData?: string) => {
    setIsLoading(true);
    setError(null);
    setProgress(0);
    ...
    const steps = [
      { pct: 5, label: "Initializing...", duration: 200 },
      { pct: 15, label: "Reading inputs...", duration: 400 },
      { pct: 25, label: "Reading inputs...", duration: 500 },
      { pct: 35, label: "Fetching job data...", duration: 600 },
      { pct: 45, label: "Fetching job data...", duration: 600 },
      { pct: 55, label: "Comparing skills...", duration: 800 },
      { pct: 65, label: "Comparing skills...", duration: 800 },
      { pct: 75, label: "Scoring fit...", duration: 800 },
      { pct: 85, label: "Writing report...", duration: 800 },
      { pct: 90, label: "Writing report...", duration: 500 },
      { pct: 95, label: "Finalizing...", duration: 400 },
    ];
    ...
    try {
      const data = await analyzeResumeAndJob(resume, targetType, targetContent, githubData);
      ...
      setError("Analysis failed. Please ensure your inputs are descriptive and try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={`font-sans text-base antialiased min-h-screen flex flex-col relative transition-colors duration-300 ${isDark ? 'dark app-background text-white' : 'app-background text-slate-800'}`}>
      {/* TopNavBar */}
      <nav className={cn(
        ...
        currentScreen === 'blueprint' ? "h-28 rounded-2xl" : "h-14 rounded-full"
      )}>
        <div className="grid grid-cols-[1fr_auto_1fr] items-center px-8 h-14 w-full z-10 border-b border-black/5 dark:border-white/5">
          <div className="flex items-center">
            <button onClick={() => setCurrentScreen('landing')} className="flex items-center outline-none">
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button onClick={() => setCurrentScreen('workspace')} className="hidden md:block bg-purple-600 hover:bg-purple-700 text-white px-4 py-1.5 rounded-full font-sans text-xs font-bold tracking-tight hover:shadow-[0_0_15px_rgba(147,51,234,0.4)] transition-all shadow-sm border border-purple-400/20">
              GET STARTED
            </button>
            <button 
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed top-20 left-1/2 -translate-x-1/2 w-[95%] max-w-sm rounded-2xl dark:bg-[#151515]/95 bg-[#fafafa]/95 backdrop-blur-xl border border-black/10 dark:border-white/10 p-4 flex flex-col gap-2 shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-200">
          <button
            onClick={() => { setCurrentScreen('workspace'); setIsMobileMenuOpen(false); }}
          ...
          <button
            onClick={() => { setCurrentScreen('blueprint'); setIsMobileMenuOpen(false); }}
            disabled={!analysisData}
          ...
          <button
            onClick={() => { setCurrentScreen('how-it-works'); setIsMobileMenuOpen(false); }}
      ...
      <main className="flex-grow relative flex flex-col items-center w-full pt-16">
        {currentScreen === 'landing' && <LandingScreen onStartMapping={() => setCurrentScreen('workspace')} isDark={isDark} />}
        ...
        {currentScreen === 'dashboard' && analysisData && <DashboardScreen data={analysisData} onViewBlueprint={handleViewBlueprint} onNewAnalysis={resetAnalysis} />}
        {currentScreen === 'blueprint' && analysisData && <BlueprintScreen data={analysisData} initialProjectIndex={selectedProjectIndex} onBack={() => setCurrentScreen('dashboard')} scrollProgress={scrollProgress} onProjectChange={setSelectedProjectIndex} />}
        {currentScreen === 'how-it-works' && <HowItWorksScreen onStartMapping={() => setCurrentScreen('workspace')} />}
      </main>

      {/* Footer */}
      {(currentScreen === 'landing' || currentScreen === 'workspace' || currentScreen === 'how-it-works') && (
        <footer className="w-full border-t border-black/10 dark:border-white/10 py-6 md:py-8 dark:bg-black/40 bg-white/40 backdrop-blur-md mt-auto z-10 relative flex-shrink-0 transition-colors">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center px-8 gap-4 md:gap-0">
            ...
            <div className="flex gap-6">
              <button onClick={() => setActiveFooterModal('privacy')} className="dark:text-white/50 text-black/50 hover:text-black dark:text-white dark:hover:text-white text-xs transition-colors">Privacy</button>
              <button onClick={() => setActiveFooterModal('terms')} className="dark:text-white/50 text-black/50 hover:text-black dark:text-white dark:hover:text-white text-xs transition-colors">Terms</button>
              <button onClick={() => setActiveFooterModal('changelog')} className="dark:text-white/50 text-black/50 hover:text-black dark:text-white dark:hover:text-white text-xs transition-colors">Changelog</button>
              <button onClick={() => setActiveFooterModal('status')} className="dark:text-white/50 text-black/50 hover:text-black dark:text-white dark:hover:text-white text-xs transition-colors">Status</button>
            </div>
      ...
      {/* Footer Modals */}
      {activeFooterModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 dark:bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setActiveFooterModal(null)}>
          <div 
          ...
          >
            ...
            {activeFooterModal === 'privacy' && (
              <div className="prose prose-sm dark:prose-invert">
                <h2 className="text-2xl font-bold mb-6 dark:text-white text-black">Privacy Policy</h2>
                <p className="dark:text-white/70 text-black/70 mb-4 font-medium">Effective Date: May 2026</p>
                <h3 className="text-lg font-bold mt-6 mb-2 dark:text-white text-black">1. Data Collection</h3>
                <p className="dark:text-white/70 text-black/70 mb-4">We collect your resume and target job descriptions purely to perform the requested analysis. No data is stored long-term without explicit consent.</p>
                <h3 className="text-lg font-bold mt-6 mb-2 dark:text-white text-black">2. AI Processing</h3>
                <p className="dark:text-white/70 text-black/70 mb-4">Your information is securely processed using advanced AI models. We do not use your inputs to train public models.</p>
                <h3 className="text-lg font-bold mt-6 mb-2 dark:text-white text-black">3. Third-party Logging</h3>
                <p className="dark:text-white/70 text-black/70 mb-4">We use anonymous telemetry and error logging to ensure the quality of our service. Personal Identifiable Information (PII) is obfuscated before transit.</p>
              </div>
            )}

            {activeFooterModal === 'terms' && (
              <div className="prose prose-sm dark:prose-invert">
                <h2 className="text-2xl font-bold mb-6 dark:text-white text-black">Terms of Service</h2>
                <p className="dark:text-white/70 text-black/70 mb-4 font-medium">Effective Date: May 2026</p>
                <h3 className="text-lg font-bold mt-6 mb-2 dark:text-white text-black">1. Acceptance</h3>
                <p className="dark:text-white/70 text-black/70 mb-4">By using StochastIQ or Hexo, you agree to these minimum viable terms. Do not use the service for illegal or malicious purposes.</p>
                <h3 className="text-lg font-bold mt-6 mb-2 dark:text-white text-black">2. Service Limitations</h3>
                <p className="dark:text-white/70 text-black/70 mb-4">The career blueprints generated are suggestions based on AI inferences, not guaranteed career advice. Use them as educational frameworks.</p>
                <h3 className="text-lg font-bold mt-6 mb-2 dark:text-white text-black">3. Warranty</h3>
                <p className="dark:text-white/70 text-black/70 mb-4">This tool is provided "as is" without warranty of any kind. We are not liable for employment outcomes influenced by this tool.</p>
              </div>
            )}

            {activeFooterModal === 'changelog' && (
              <div className="prose prose-sm dark:prose-invert">
                <h2 className="text-2xl font-bold mb-6 dark:text-white text-black">Changelog</h2>
                <div className="mb-6 pb-6 border-b dark:border-white/10 border-black/10">
                  <span className="inline-block px-2 py-1 text-xs font-bold bg-black dark:bg-white text-white dark:text-black rounded mb-2">v.2.1.0</span>
                  <h3 className="text-lg font-bold mt-1 mb-2 dark:text-white text-black">PDF Export & Layout Refinements</h3>
                  <p className="dark:text-white/70 text-black/70 mb-2 mt-2">Added clean "Export PDF" function to blueprints, stripped out UI elements from print formats, and introduced a responsive mobile menu structure.</p>
                </div>
                <div className="mb-6 pb-6 border-b dark:border-white/10 border-black/10">
                  <span className="inline-block px-2 py-1 text-xs font-bold bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 rounded mb-2">v.2.0.0</span>
                  <h3 className="text-lg font-bold mt-1 mb-2 dark:text-white text-black">The "Blueprint" Update</h3>
                  <p className="dark:text-white/70 text-black/70 mb-2 mt-2">Introduced full structured learning blueprints parsing from Gemini models. Multi-project switching mechanism added for multiple gap assessments.</p>
                </div>
                <div>
                  <span className="inline-block px-2 py-1 text-xs font-bold bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 rounded mb-2">v.1.0.0</span>
                  <h3 className="text-lg font-bold mt-1 mb-2 dark:text-white text-black">Initial Launch</h3>
                  <p className="dark:text-white/70 text-black/70 mb-2 mt-2">Core analyzer mapping experience created with dark/light mode integration.</p>
                </div>
              </div>
            )}
            ...
            {activeFooterModal === 'status' && (
              <div className="prose prose-sm dark:prose-invert">
                <h2 className="text-2xl font-bold mb-6 dark:text-white text-black">System Status</h2>
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl border border-green-500/30 bg-green-500/10 mb-6">
                  ...
                  <div className="flex-col">
                    <span className="font-bold text-green-700 dark:text-green-400 block">All Systems Operational</span>
                    <span className="text-xs text-green-600/80 dark:text-green-400/80">Updated just now</span>
                  </div>
                </div>

                <div className="space-y-4">
                  <div className="flex items-center justify-between p-4 bg-black/5 dark:bg-white/5 rounded-lg">
                    <span className="font-medium dark:text-white text-black">Analysis Engine (Gemini)</span>
                    <span className="text-sm text-green-600 dark:text-green-400">Operational</span>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-black/5 dark:bg-white/5 rounded-lg">
                    <span className="font-medium dark:text-white text-black">Web Interface</span>
                    <span className="text-sm text-green-600 dark:text-green-400">Operational</span>
                  </div>
                  <div className="flex items-center justify-between p-4 bg-black/5 dark:bg-white/5 rounded-lg">
                    <span className="font-medium dark:text-white text-black">PDF Export Service</span>
                    <span className="text-sm text-green-600 dark:text-green-400">Operational</span>
                  </div>
                </div>
              </div>

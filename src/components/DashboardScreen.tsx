...
export function DashboardScreen({ data, onViewBlueprint, onNewAnalysis }: DashboardScreenProps) {
  if (!data) return null;

  // Calculate match percentage and find max gap for node focus
  const totalActual = data.skills.radarData.reduce((acc: number, d: any) => acc + (d.actual || d.user || 0), 0);
  const totalTarget = data.skills.radarData.reduce((acc: number, d: any) => acc + (d.target || 100), 0);
  const matchPercentage = data.matchPercentage !== undefined ? data.matchPercentage : Math.round((totalActual / totalTarget) * 100);

  // Find the axis with the largest gap for the glowing focal point
  const radarData = data.skills.radarData.map((d: any) => ({
    ...d,
  ...
  const maxGapNode = [...radarData].sort((a: any, b: any) => (b.target - b.actual) - (a.target - a.actual))[0];

  const getClassification = (score: number) => {
    if (score >= 80) return { label: 'STRONG MATCH', color: 'bg-purple-900 dark:bg-purple-50 text-white dark:text-purple-900 border-purple-200 dark:border-purple-400/30', hover: 'Highly qualified with excellent core competency match.' };
    if (score >= 63) return { label: 'COMPETITIVE', color: 'bg-purple-900 dark:bg-purple-50 text-white dark:text-purple-900 border-purple-200 dark:border-purple-400/30', hover: 'Solid candidate with minimal gaps to bridge.' };
    if (score >= 45) return { label: 'DEVELOPING', color: 'bg-purple-100/50 dark:bg-purple-900/30 text-purple-900 dark:text-purple-100 border-purple-200 dark:border-purple-500/20', hover: 'Has foundational skills but needs specific upskilling.' };
    if (score >= 25) return { label: 'EMERGING', color: 'bg-purple-100/50 dark:bg-purple-900/30 text-purple-900 dark:text-purple-100 border-purple-200 dark:border-purple-500/20', hover: 'Initial signs of potential, but requires significant training.' };
    return { label: 'MISALIGNED', color: 'bg-purple-900 dark:bg-purple-50 text-white dark:text-purple-900 border-purple-200 dark:border-purple-400/30', hover: 'Significant skill gaps exist for this role.' };
  };
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const dataPoint = radarData.find((d: any) => d.attribute === label || d.axis === label);
      ...
      const getRanges = (tierType: string) => {
        if (tierType === 'core') {
          return [
            { label: 'Entry', min: 0, max: 45, color: 'bg-purple-200 dark:bg-purple-900/40' },
            { label: 'Jr', min: 45, max: 65, color: 'bg-purple-300 dark:bg-purple-800/50' },
            { label: 'Mid', min: 65, max: 80, color: 'bg-purple-500 dark:bg-purple-600/70' },
            { label: 'Sr', min: 80, max: 92, color: 'bg-purple-700 dark:bg-purple-400/80' },
            { label: 'Staff+', min: 92, max: 100, color: 'bg-purple-900 dark:bg-purple-100' }
          ];
        }
        if (tierType === 'supporting') {
          return [
            { label: 'Entry', min: 0, max: 35, color: 'bg-purple-200 dark:bg-purple-900/40' },
            { label: 'Jr', min: 35, max: 55, color: 'bg-purple-300 dark:bg-purple-800/50' },
            { label: 'Mid', min: 55, max: 70, color: 'bg-purple-500 dark:bg-purple-600/70' },
            { label: 'Sr', min: 70, max: 85, color: 'bg-purple-700 dark:bg-purple-400/80' },
            { label: 'Staff+', min: 85, max: 100, color: 'bg-purple-900 dark:bg-purple-100' }
          ];
        }
        // Contextual
        return [
          { label: 'Entry', min: 0, max: 30, color: 'bg-purple-200/50 dark:bg-purple-900/40' },
          { label: 'Jr', min: 30, max: 45, color: 'bg-purple-300/60 dark:bg-purple-800/50' },
          { label: 'Mid', min: 45, max: 60, color: 'bg-purple-400/70 dark:bg-purple-700/60' },
          { label: 'Sr', min: 60, max: 75, color: 'bg-purple-600/80 dark:bg-purple-500/80' },
          { label: 'Staff+', min: 75, max: 100, color: 'bg-purple-800 dark:bg-purple-300' }
        ];
      };

      const ranges = getRanges(tier);
      const currentLevel = ranges.find(r => userScore >= r.min && userScore <= r.max) || ranges[0];
      ...
      return (
        <div className="dark:bg-black/70 bg-white/70 backdrop-blur-md border dark:border-white/10 border-black/20 p-4 rounded-xl shadow-xl z-50 max-w-[300px]">
          <div className="flex items-center justify-between gap-4 mb-3 pb-2 border-b dark:border-white/10 border-black/20">
            <p className="dark:text-white text-black font-bold uppercase tracking-wider text-[11px] font-mono">{label}</p>
            <span className={cn(
            ...
            {payload.map((entry: any, index: number) => (
              <div key={index} className="flex items-center justify-between gap-6 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="dark:text-white text-black font-semibold">{entry.name}</span>
                </div>
          ...
          <div className="pt-3 border-t dark:border-white/10 border-black/20">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[10px] uppercase font-mono font-bold text-black/60 dark:text-white/60">Benchmark: {currentLevel.label} level</span>
            </div>
              ...
              {ranges.map((r, i) => (
                <div 
                  ...
                  className={cn(
                    ...
                    userScore >= r.min && userScore <= r.max ? "text-black dark:text-white font-bold underline" : "text-black/40 dark:text-white/40 font-medium"
                  )}
      ...
      <div className="w-full max-w-6xl flex flex-col gap-8 pb-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 relative z-10">
            ...
            <div className="lg:col-span-4 border-r border-purple-100 dark:border-purple-500/20 p-8 flex flex-col gap-8 dark:bg-purple-900/10 bg-white/60 backdrop-blur-md">
              ...
              <div className="relative pl-6 border-l-4 border-purple-600 dark:border-purple-400 py-2">
                <span className="text-[10px] font-mono dark:text-purple-300/50 text-purple-900/50 uppercase tracking-widest block mb-1">Target Role</span>
                <h3 className="text-2xl font-bold dark:text-purple-50 text-slate-900 tracking-tight leading-tight">{data.targetRole || "Technical Architect"}</h3>
              </div>

              {/* Current Match Card */}
              <div className="relative pl-6 border-l-4 border-purple-600 dark:border-purple-400 py-2">
                <span className="text-[10px] font-mono dark:text-purple-300/50 text-purple-900/50 uppercase tracking-widest block mb-1">Current Match</span>
                <div className="flex items-center gap-4">
                  <span className="text-5xl font-black dark:text-purple-50 text-slate-900 tracking-tighter">{matchPercentage}%</span>
                  <div className="group/tag relative cursor-help flex">
                    <span className={cn("text-[10px] font-mono font-bold px-2.5 py-1 rounded border uppercase tracking-wider", matchClassification.color)}>
              ...
              <div className="pt-6 border-t border-purple-100 dark:border-purple-500/20">
                <span className="text-[10px] font-mono dark:text-purple-300/50 text-purple-900/50 uppercase tracking-widest block mb-4">Critical Gaps</span>
                <div className="space-y-4">
                  {radarData
                    .filter((s: any) => (s.target - s.actual) > 15)
                    .sort((a: any, b: any) => ((b.target - b.actual) / (b.target || 100)) - ((a.target - a.actual) / (a.target || 100)))
                    .slice(0, 4)
                    .map((skill: any, i: number) => (
                    <div key={i} className="flex items-center justify-between group">
                      <span className="dark:text-blue-100/60 text-slate-600 text-xs font-semibold group-hover:text-purple-900 dark:group-hover:text-purple-100 transition-colors uppercase tracking-wider">{skill.attribute}</span>
                      <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-900 dark:text-purple-100 text-[10px] font-mono font-bold px-2 py-0.5 rounded border border-purple-200 dark:border-purple-500/20">
                        -{Math.round(((skill.target - skill.actual) / (skill.target || 100)) * 100)}%
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="w-full h-[450px]">
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="80%" data={radarData}>
                    ...
                    <PolarAngleAxis 
                      dataKey="attribute" 
                      tick={{ fill: "var(--ink-color)", fontSize: 10, fontWeight: 600, fontFamily: 'monospace' }}
                    ...
                    <Radar
                      ...
                      fillOpacity={0.15}
                      dot={(props: any) => {
                        const isMaxGap = props.payload.attribute === maxGapNode.attribute;
                        return (
                          <circle 
                            cx={props.cx} 
                            cy={props.cy} 
          ...
          <div className="glass-card rounded-xl overflow-hidden group border border-purple-100 dark:border-purple-500/20 transition-all hover:border-purple-300 dark:hover:border-purple-500/40 bg-white/30 dark:bg-purple-900/10">
            <div className="bg-purple-600 dark:bg-purple-500/20 border-b border-purple-400 dark:border-purple-500/20 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-bold text-white uppercase tracking-[0.2em] font-mono">Core Competences</h3>
              </div>
              <div className="w-2 h-2 rounded-full bg-white/40 animate-pulse"></div>
            </div>
            <div className="p-8 space-y-5 bg-white/30 dark:bg-purple-900/10 backdrop-blur-md">
              {data.skills.strengths.map((str: { name: string; matchRatio: number }, i: number) => {
                return (
          ...
          <div className="glass-card rounded-xl overflow-hidden group border border-purple-100 dark:border-purple-500/20 transition-all hover:border-purple-300 dark:hover:border-purple-500/40 bg-white/30 dark:bg-purple-900/10">
            <div className="bg-purple-800 dark:bg-purple-900/40 border-b border-purple-600 dark:border-purple-500/30 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-bold text-white uppercase tracking-[0.2em] font-mono">Areas for Improvement</h3>
              </div>
              <div className="w-2 h-2 rounded-full bg-white/40 animate-pulse"></div>
            </div>
            <div className="p-8 space-y-5 bg-white/30 dark:bg-purple-900/10 backdrop-blur-md">
        ...
        <div className="mt-8 pt-12 border-t border-purple-100 dark:border-purple-500/20">
           <div className="flex items-center gap-3 mb-10">
              ...
              <div>
                <h3 className="text-xl font-bold dark:text-purple-50 text-slate-900 tracking-tight">Personalized Projects</h3>
                <p className="text-xs dark:text-purple-300/50 text-purple-900/50 font-mono uppercase tracking-[0.2em] mt-1">Closing identified gaps through execution</p>
              </div>
           </div>
               ...
               >
                 ...
                 <div className="flex items-center justify-between mb-6 relative z-10">
                    ...
                    <div className="flex gap-1.5">
                       {Array.from({ length: 5 }).map((_, i) => (
                         <div key={i} className={`w-1.5 h-1.5 rounded-full ${i < project.difficultyLevel ? 'bg-purple-600/50 dark:bg-purple-400/50' : 'bg-purple-100 dark:bg-purple-900/80'}`}></div>
                       ))}
                    </div>
                 </div>
                 ...
                 <div className="flex flex-wrap gap-2 mb-8 relative z-10">
                   {project.techStack.slice(0, 3).map((tech: string, i: number) => (
                     <span key={i} className="px-2.5 py-1 bg-purple-50 dark:bg-purple-900/30 text-purple-900 dark:text-purple-100 border border-purple-200 dark:border-purple-500/20 rounded text-[10px] font-mono uppercase tracking-wider">
                       {tech}
                     </span>
                   ))}
                   {project.techStack.length > 3 && <span className="text-[10px] dark:text-purple-300 text-purple-900/50 self-center">+{project.techStack.length - 3}</span>}
                 </div>
